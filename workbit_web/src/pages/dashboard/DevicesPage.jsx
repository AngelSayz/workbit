    ...
    <div className="w-full h-full p-4 sm:p-6 space-y-6 overflow-x-hidden">
      ...
      <div className="space-y-4">
        ...
        <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
          <div className="flex items-center space-x-4">
            ...
            >
              ...
              <option value="access_control">Acceso</option>
              <option value="security">Seguridad</option>
              <option value="monitoring">Monitoreo</option>
            </select>
          </div>
          <div className="text-sm text-gray-600">
            {devices.filter(d => selectedType === 'all' || d.type === selectedType).length} dispositivos
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {devices
            .filter(device => selectedType === 'all' || device.type === selectedType)
            .map((device, index) => (
              <motion.div
                key={device.id}
                initial={{ opacity: 0, y: 20 }}
